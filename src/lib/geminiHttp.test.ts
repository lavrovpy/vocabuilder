...
describe("throwForHttpError", () => {
  ...
  it("maps 429 to rate-limited with status + body", async () => {
  ...
  it("keeps rate-limited mapping when the 429 body is not JSON", async () => {
    await expect(throwForHttpError(makeResponse(429, "server said no"), "translate", "m")).rejects.toMatchObject({
      cause: {
        kind: "rate-limited",
        body: "server said no",
        rateLimit: undefined,
  ...
  it.each([500, 503])("maps other non-ok %d to request-failed with status + body", async (status) => {
