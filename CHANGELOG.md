...
- Added an `API Base URL` preference so translation and pronunciation can be routed through any endpoint that speaks the native Gemini REST protocol, such as a proxy or gateway. Your API key is sent to the host you configure, so plain `http` is accepted only for local addresses.
