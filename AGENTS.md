...
- Use the Node version in `.nvmrc` — **Node 22.22.2 / npm 10.9.7** — which matches the current Raycast extension toolchain. Run `nvm use` or `mise exec node@22.22.2 -- ...` before installing dependencies. Do not regenerate `package-lock.json` under npm 11.
...
- Always `nvm use` or `mise exec node@22.22.2 -- ...` before any `npm install`. A stray `npm install` under npm 11 silently strips the node back out and re-breaks CI.
- If the lockfile was touched on the wrong npm, regenerate it: `npx -y npm@10.9.7 install --package-lock-only`, then verify with `npx -y npm@10.9.7 ci --dry-run`.
- Do **not** add `engine-strict` to force this — it enforces *dependency* engines too, and transitive packages may temporarily declare stricter engines than Raycast's published compatibility window.
